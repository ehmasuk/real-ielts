...
export default function SpeakingPage() {
  ...
  const { data: tests = [], isLoading: testsLoading } = useQuery({
    queryKey: ["public", "tests", "speaking"],
    queryFn: () => fetchPublicTestsList({ skill: "speaking" }),
  })
  ...
  return (
    <div className="mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 py-12 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <div className="inline-flex items-center gap-1.5 rounded-full bg-violet-500/10 px-3 py-1 text-xs font-semibold text-violet-600 dark:text-violet-400 mb-3">
            <Mic className="h-3.5 w-3.5" />
            <span>Speaking Prep Room</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-foreground">
            Official Cambridge Speaking Library
          </h1>
          <p className="text-sm text-muted-foreground mt-2 max-w-2xl leading-relaxed">
            Select any test to view Speaking questions. Each test contains 3 parts: an interview, a cue card task, and a discussion.
          </p>
        </div>
      ...
      <div className="mb-10">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-card/50 border border-border/30 rounded-xl py-2 pl-9 pr-4 text-sm outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/30 transition-all placeholder:text-muted-foreground/60 text-foreground"
          />
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col gap-8">
          <BookCardSkeleton accentColor="bg-violet-500/10" />
          <BookCardSkeleton accentColor="bg-violet-500/10" />
        </div>
        ...
        <div className="flex flex-col gap-8">
          {filteredBooks.map((book) => {
            ...
            return (
              <div
                key={book._id}
                className="group flex flex-col md:flex-row rounded-2xl border border-border/40 bg-card/30 hover:bg-card/70 hover:border-violet-500/30 transition-all duration-300 shadow-sm hover:shadow-md overflow-hidden"
              >
