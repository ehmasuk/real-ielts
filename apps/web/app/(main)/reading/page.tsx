...
const generateBooks = (): Book[] => {
  ...
  return Array.from({ length: 21 }, (_, index) => {
    ...
    const color = gradients[bookNum % gradients.length] ?? "from-purple-600 to-purple-900"
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-2xl leading-relaxed">
            Select any passage to practice Academic and General Reading. Each full test contains 3 passages with multiple choice, matching headings, and true/false tasks.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-10 bg-card/50 backdrop-blur-sm border border-border/40 p-3 rounded-2xl shadow-sm">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-background/50 border border-border/30 rounded-xl py-2 pl-9 pr-4 text-sm outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/30 transition-all placeholder:text-muted-foreground/60 text-foreground"
          />
        </div>
                ...
                {[1, 2, 3, 4].map((testNum) => (
                  <div key={testNum} className="flex flex-col justify-between space-y-3 bg-muted/10 dark:bg-muted/5 p-4 rounded-xl border border-border/20">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-foreground">
                      <Play className="h-3.5 w-3.5 text-purple-500 fill-purple-500/20 shrink-0" />
                      <span>Practice Test {testNum}</span>
                    </div>
                    
                    {/* Stack for the 3 passages */}
                    <div className="flex flex-col gap-2">
                      {[1, 2, 3].map((passageNum) => (
                        <Link 
                          key={passageNum}
                          href={`/test/cambridge-${book.id}/test-${testNum}/part-${passageNum}/reading`}
