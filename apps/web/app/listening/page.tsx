...
import { Search, Headphones, BookOpen, Filter, Play } from "lucide-react"
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-10 bg-card/50 backdrop-blur-sm border border-border/40 p-3 rounded-2xl shadow-sm">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-background/50 border border-border/30 rounded-xl py-2 pl-9 pr-4 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-all placeholder:text-muted-foreground/60 text-foreground"
          />
        </div>
              ...
              <div className="flex-1 p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[1, 2, 3, 4].map((testNum) => (
                  <div key={testNum} className="flex flex-col justify-between space-y-3 bg-muted/10 dark:bg-muted/5 p-4 rounded-xl border border-border/20">
                    ...
                    {/* Stack for the 4 universal parts */}
                    <div className="flex flex-col gap-2">
                      {[1, 2, 3, 4].map((partNum) => (
                        <Link 
                          key={partNum}
                          href={`/test/cambridge-${book.id}/test-${testNum}/part-${partNum}/listening`}
                          className="flex items-center justify-between rounded-lg border border-border/40 bg-background/50 hover:bg-indigo-600 hover:border-indigo-600 hover:text-white px-3 py-2 text-[11px] font-semibold text-muted-foreground transition-all shadow-sm cursor-pointer"
                        >
