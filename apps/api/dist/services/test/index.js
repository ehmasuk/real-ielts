...
const testServices = {
    ...
    submitFullTest: async (userId, testId, skill, allAnswers, timeTaken, mode = "practice") => {
        ...
        await UserFullTestResult.findOneAndUpdate({
            userId: new mongoose.Types.ObjectId(userId),
            testId: new mongoose.Types.ObjectId(testId),
            skill,
        }, {
    ...
    getFullTestResult: async (userId, testId, skill) => {
        const [result, test] = await Promise.all([
            UserFullTestResult.findOne({
                userId: new mongoose.Types.ObjectId(userId),
                testId: new mongoose.Types.ObjectId(testId),
                skill,
            }).lean(),
        ...
        if (!result)
            return null;
        const sections = (test?.contentJson?.sections ?? []).map((s) => ({
            title: s.title,
    ...
    getUserFullTestResults: async (userId) => {
        const results = await UserFullTestResult.find({ userId: new mongoose.Types.ObjectId(userId) }, { testId: 1, skill: 1, totalScore: 1, totalMax: 1, _id: 0 }).lean();
        return results.map((r) => ({
            testId: r.testId.toString(),
            skill: r.skill,
