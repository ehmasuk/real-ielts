...
async function seed() {
  ...
  // ---- Books ----
  const booksData = [
    { number: 17, title: "Cambridge IELTS 17", slug: "cambridge-ielts-17", status: "published" as const },
    { number: 18, title: "Cambridge IELTS 18", slug: "cambridge-ielts-18", status: "published" as const },
    { number: 19, title: "Cambridge IELTS 19", slug: "cambridge-ielts-19", status: "published" as const },
  ];
  ...
  await DrillSchemaModel.findOneAndUpdate(
    { drillId: "spelling_challenge" },
    { drillId: "spelling_challenge", schema: spellingSchema, version: 1 },
    { upsert: true }
  );
  ...
  await DrillSchemaModel.findOneAndUpdate(
    { drillId: "sentence_dictation" },
    { drillId: "sentence_dictation", schema: sentenceDictationSchema, version: 1 },
    { upsert: true }
  );
...
function makeListeningContent(bookNumber: number, testNumber: number) {
  return {
...
function makeListeningAnswers(testNumber: number) {
...
function makeReadingAnswers(testNumber: number) {
  const answers: Record<string, any> = {
    q_1: "FALSE",
    q_2: "FALSE",
    q_3: "FALSE",
