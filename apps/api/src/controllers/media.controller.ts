...
export const getAllMedia = catchAsync(async (req: CustomRequest, res: Response) => {
  const assets = await mediaServices.getAll(req.user!.id);
  res.status(200).json(assets);
});

export const createMedia = catchAsync(async (req: CustomRequest, res: Response) => {
...
export const updateMedia = catchAsync(async (req: CustomRequest, res: Response) => {
...
export const deleteMedia = catchAsync(async (req: CustomRequest, res: Response) => {
  const id = req.params.id;
  if (!id) {
    res.status(400).json({ message: "Missing media id" });
    return;
