...
const getSchema = catchAsync(async (req: CustomRequest, res: Response) => {
  const { drillId } = req.params
  if (!drillId) throw newError({ message: "drillId is required", statusCode: 400 })

  const doc = await drillSchemaServices.get(drillId)
  if (!doc) throw newError({ message: "Schema not found", statusCode: 404 })

  successResponse({ res, data: { schema: doc.schema, version: doc.version } })
})

const getAllSchemas = catchAsync(async (req: CustomRequest, res: Response) => {
  const docs = await drillSchemaServices.getAll()
  successResponse({ res, data: docs.map((d) => ({ drillId: d.drillId, version: d.version })) })
})

const getAllSchemasPublic = catchAsync(async (req: CustomRequest, res: Response) => {
  const docs = await drillSchemaServices.getAll()
  successResponse({ res, data: docs.map((d) => ({ drillId: d.drillId, schema: d.schema, version: d.version })) })
})

const getSchemaAdmin = catchAsync(async (req: CustomRequest, res: Response) => {
  const { drillId } = req.params
  if (!drillId) throw newError({ message: "drillId is required", statusCode: 400 })
...
const updateSchema = catchAsync(async (req: CustomRequest, res: Response) => {
  const { drillId } = req.params
  if (!drillId) throw newError({ message: "drillId is required", statusCode: 400 })

  const { schema } = req.body
  if (!schema || typeof schema !== "object") throw newError({ message: "schema object is required", statusCode: 400 })
