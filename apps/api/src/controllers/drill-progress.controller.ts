...
const getProgress = catchAsync(async (req: CustomRequest, res: Response) => {
...
const updateProgress = catchAsync(async (req: CustomRequest, res: Response) => {
  const userId = req.user!.id
  const { drillId } = req.params
  if (!drillId) throw newError({ message: "drillId is required", statusCode: 400 })
...
const resetProgress = catchAsync(async (req: CustomRequest, res: Response) => {
  const userId = req.user!.id
  const { drillId } = req.params
  if (!drillId) throw newError({ message: "drillId is required", statusCode: 400 })
